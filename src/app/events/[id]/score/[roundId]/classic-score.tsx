...
// last tap, so fast scoring no longer fires a racing POST per tap.
...
export function ClassicEventScore({ id, roundId }: { id: string; roundId: string }) {
  ...
  // Debounced score saves — coalesce a fast burst of taps on one cell into a
  // single request instead of several racing POSTs (the cause of "Save failed").
  ...
  // Flush queued saves before the tab is hidden/closed and on unmount.
  useEffect(() => {
    const onHide = () => {
  ...
  // A failed save means the server kept the old value, so drop the optimistic
  ...
  // Changing holes flushes any pending save immediately.
