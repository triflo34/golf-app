...
export function V2Poker({ id }: { id: string }) {
  ...
  const swapBtn =
    "rounded border border-[var(--v2-border)] px-2 py-1 text-xs text-[var(--v2-text)] hover:border-[var(--v2-gold)]/50 disabled:opacity-50";
      ...
      <div className="mx-auto max-w-lg space-y-5 px-4 py-4 pb-28">
        ...
        {mySwaps.length > 0 &&
          ...
          (() => {
            ...
            return (
              <section className="rounded-xl border border-[var(--v2-gold)]/45 bg-[var(--v2-gold)]/10 p-3">
                <div className="text-sm font-semibold text-[var(--v2-gold)]">
                  Pending decision
                  {remaining > 0 && (
                    <span className="ml-2 text-xs font-normal text-[var(--v2-text-dim)]">
                      ({remaining} more after this)
                    </span>
                  )}
                </div>
                <div className="mt-2 rounded-md border border-[var(--v2-border)] bg-[var(--v2-surface-2)] p-2">
                  {s.incoming_card ? (
                    <div>
                      <div className="mb-1 text-xs text-[var(--v2-text-dim)]">
                        New card drawn — keep it and discard one, or skip:
                      </div>
                      <div className="flex items-center gap-2">
                        <CardFace card={s.incoming_card} />
                        <span className="text-xs text-[var(--v2-text-dim)]">incoming</span>
                      </div>
                      <div className="mt-2 flex flex-wrap gap-1">
                        ...
                        <button
                          ...
                          className="rounded border border-[var(--v2-border)] px-2 py-1 text-xs text-[var(--v2-text-dim)] disabled:opacity-50"
                        >
                          Skip (discard new)
                        </button>
                      </div>
                    ...
                    <div>
                      <div className="mb-2 text-xs text-[var(--v2-text-dim)]">
                      ...
                      <div className="flex flex-wrap gap-1">
                        {myHand.cards.map((c, idx) => (
                          <button
                            ...
                            className="rounded border border-[var(--v2-border)] px-2 py-1 text-xs text-[var(--v2-text)] hover:border-[var(--v2-red)] disabled:opacity-50"
                          >
                            Discard {c.rank}
                            {SUIT_GLYPH[c.suit]}
                          </button>
        ...
        {myHand && (
          <section>
            ...
            <div className="v2-card">
              ...
              <div className="mt-2 text-[11px] text-[var(--v2-text-dim)]">
                {reorderPick == null
