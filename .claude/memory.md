...
## Recent Changes
- Event page upgrades (2026-06-04): poker payout/winner, payout settlement, leaderboard polish. No schema change.
  - **Poker winner**: new `POST /api/events/[id]/side-games/poker/winner` (organizer-only, reuses `side_game_results` with `player_id`, rank 1, payout = pot; `player_id:null` clears). `loadEventStandings` now returns `poker: PokerStandings` (hand-holders w/ card_count + winner + payout). Event detail (classic + v2) Side Games tab has a Win/Unset picker like scramble; Payouts `FinalPayoutWinner` shows the real poker winner (replaced the "not yet in MVP" note).
  - **Settlement**: `loadEventStandings` computes `settlement: Settlement` (per-player paid/won/net + minimal "A pays B $X" greedy transfer list). Each pool (entry pot + each side game *with a winner*) funded by equal buy-in across all players, paid to winner(s); pools without a winner are skipped so the books balance to zero. Entry pot → overall low score (ties split). Surfaced as a "Settle up" card in the $ tab (completed events only), with the equal-buy-in / low-score-takes-entry assumption stated inline.
  - **Leaderboard polish**: event Board tab now has a 👑 on the leader, `T`-prefixed competition-rank ties (same strokes+through share rank), and tap-to-expand per-round breakdown. New `computeRanks` helper in both detail files.
  - Side-game winner picks (poker + scramble) now call a `reload` callback so they refresh immediately even after the event is completed (polling off). Lint: only the pre-existing `set-state-in-effect` findings remain; no new ones.
