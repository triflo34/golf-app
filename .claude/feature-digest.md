...
- **Fully ported (2026-06-02):** `/admin`, `/courses/[id]/edit-holes`, and the course-detail admin panels (rename / GolfCourseAPI refresh / relink) all now have mode-gated v2 variants — no classic-only surfaces remain. The shared inner forms (`RoundForm`, `ScorecardRoundForm`, `ScorecardGrid`, `PlayerPicker`, `MatchBuilder`) already carry `variant` props. Also: v2 Stats Board gained placement chips + tap-to-expand hole stats + a points explainer, and the v2 home recent-rounds list now shows every player's score inline.
